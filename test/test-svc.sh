...
list=$(@svc-list)
assert contains "$list" "svc-a"
assert contains "$list" "svc-b"

t "svc-list shows window IDs"
assert contains "$list" "$win1"
assert contains "$list" "$win2"

t "svc-list shows worktree names"
assert contains "$list" "wt_a"
assert contains "$list" "wt_b"

t "svc-list -n filters by name"
list=$(@svc-list -n svc-a)
assert contains "$list" "svc-a"
assert_not contains "$list" "svc-b"

t "svc-list -w filters by worktree"
list=$(@svc-list -w wt_b)
assert_not contains "$list" "svc-a"
assert contains "$list" "svc-b"
...
t "svc-stop cleans up status file"
@svc-stop "$win" >/dev/null
assert_not test -f "$SRCERY_DATA/status/st_wt/st-svc"
